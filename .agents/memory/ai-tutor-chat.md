...
- **Strategy AI** POST `/parent/:id/consultant/message` is its OWN hand-written SSE endpoint (mirrors the tutor chat pattern, NOT in OpenAPI): events `{chunk}` + terminal `{done,sessionId}`. Counselor persona/prompt in `lib/counselor.ts` (`buildCounselorSystemPrompt`), model `gpt-5-mini`. Parent turns persisted append-only to `parent_chat_messages`.
