...
- **Transcribe** POST `/parent/:id/consultant/transcribe` takes `{audio:dataUrl, mimeType, language}` → `{text}`, uses `speechToText` + `ensureCompatibleFormat` from `@workspace/integrations-openai-ai-server/audio` (`gpt-4o-mini-transcribe`). Frontend records via MediaRecorder (NOT Web Speech API — the student chat uses Web Speech; the parent chat uses server transcription so it works in more browsers + honours the chosen language).
