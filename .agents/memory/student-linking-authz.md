...
- Tutor linking: single-link and bulk-link flows must persist the selected batch; the tutor picker must also be visible on the coach tab, because the same selected linked student feeds roster, progress, history, and chatbot context. Repair older null-batch links when a batch is supplied.
